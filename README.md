...
대분류 라벨(예: `RFP/공고분석`)은 프리셋의 CLAUDE.md 표가 단일 소스다. 변경 방법:

- **신규 프로젝트 전체에 적용**: 프로젝트 생성 모달의 ⚙ 프리셋 관리 → 편집 → CLAUDE.md `content`의 `### 대분류표` 표에서 라벨만 수정 → 저장. 이후 생성되는 프로젝트의 CLAUDE.md / index.md / 사이드바 라벨이 모두 새 값으로 일치한다.
- **기존 프로젝트 한 건만**: 그 프로젝트의 `CLAUDE.md` 파일을 열어 표의 라벨을 수정하고 저장. 새로고침하면 사이드바 그룹 라벨이 즉시 반영된다. 이미 생성된 `000_index.md`의 헤더(`### 1xx ...`)는 자동으로 갱신되지 않으므로, 통일이 필요하면 직접 편집한다.
- **표를 손상/삭제한 경우**: 백엔드가 내장 fallback 라벨(기본 정부지원사업 분류)을 사용한다.
